...
class RecipeSerializers(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        for y in range(lens):
            for key, value in ret['ingredients'][y]['food'][0]['nutrition'].items():
                total_nutrition += convert_mg_to_g(key, value)
            ret['kcal_total'] += ret['ingredients'][y]['food'][0]['kcal']
            ret['protein_total'] += float(ret['ingredients'][y]['food'][y]['nutrition'][0]['protein'].split(' ')[0])
            ret['protein_presentase'] += count_presentation(ret['protein_total'], total_nutrition)
            ret['lemak_total'] += float(ret['ingredients'][y]['food'][y]['nutrition'][0]['lemak'].split(' ')[0])
            ret['lemak_presentase'] += count_presentation(ret['lemak_total'], total_nutrition)
            ret['karbo_total'] += float(ret['ingredients'][y]['food'][y]['nutrition'][0]['carbo'].split(' ')[0])
